		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Sortez de votre véhicule pour faire le plein")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Appuyer sur ~g~G ~w~ pour arreter de mettre le plein. ~g~" .. price .. " ~w~$")
				...
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Vous n’avez actuellement pas assez d’argent pour acheter du carburant")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Appuyez sur ~g~G ~w~pour faire le plein de votre véhicule. $~r~0.5/~w~gallon")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Appuye sur ~g~G ~w~pour arreter de remplir ton vehicule. Currently at: " .. fuel .. "% - Jerry Can: " .. jerrycan)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsFueling then
			...
			price = price + fuelthis * 0.5 * 1.1 * 5.0
...
function DisplayHud()
	if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetSeatPedIsIn(GetPlayerPed(-1)) == -1 then
		...
		--DrawAdvancedText(0.2195 - x, 0.77 - y, 0.005, 0.0028, 0.6,"" ..fuel.." Fuel", 255, 255, 255, 255, 6, 1)
		DrawAdvancedText(0.11101, 0.945, 0.005, 0.0028, 0.6,fuel, 255, 255, 255, 255, 6, 1)
		drawRct(0.015, 0.932, 0.046, 0.03, 0, 0, 0, 100)
		DrawAdvancedText(0.1425, 0.953, 0.005, 0.0028, 0.4, "Fuel", 255, 255, 255, 255)
		--DrawAdvancedText(0.130 - x, 0.77 - y, 0.005, 0.0028, 0.6, mph, 255, 255, 255, 255, 6, 1)
		--DrawAdvancedText(0.174 - x, 0.77 - y, 0.005, 0.0028, 0.6, kmh, 255, 255, 255, 255, 6, 1)

		--DrawAdvancedText(0.148 - x, 0.7765 - y, 0.005, 0.0028, 0.4, "mp/h              km/h              Fuel", 255, 255, 255, 255, 6, 1)
	
		--DrawRect(hud_x, hud_y, 0.139, 0.008, 0, 0, 0, 150)
		--DrawRect(hud_x, hud_y, 0.145, 0.008, 80, 80, 80, 25)
		--DrawRect(hud_x, hud_y, fuel, 0.0058, 225, 146, 45, 200)
